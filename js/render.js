...
function renderPlanLog() {
  const log = state.communityChangelog;
  if (!log.length) {
    return '<div style="text-align:center;color:var(--muted);padding:40px">Noch keine Einträge.</div>';
  }

  const iconFor = field => {
    if (field.includes('Abfrage') || field.includes('Abstimmung')) return '📋';
    if (field.includes('Route')) return '🗺️';
    if (field.includes('Admin'))  return '👑';
    return '📝';
  };
...
<div class="changelog-row">
  <div class="changelog-date">${dateStr}<br>${timeStr}</div>
  <div style="display:flex;align-items:flex-start;gap:8px">
    <span style="font-size:16px;line-height:1.4">${iconFor(e.field)}</span>
    <div>
      <div class="changelog-user">${esc(e.username)}</div>
      <div class="changelog-field">${esc(e.field)}</div>
      ${e.new_value ? `<div class="changelog-val">${esc(e.new_value)}</div>` : ''}
    </div>
  </div>
</div>`;
  }).join('');

  return `<div class="plan-polls-scroll">${rows}</div>`;
}
